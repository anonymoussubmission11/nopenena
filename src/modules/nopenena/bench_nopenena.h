...
#ifndef SECP256K_NOPENI_BENCH_NOPENI_H
#define SECP256K_NOPENI_BENCH_NOPENI_H
...
void bench_nopenena_contract_t(int zero_com_flag, int n) {
    int P[] = {2, 2, 2, 2, 2, 2};
    ...
    for (int test = 0; test < 6; test++) {
        ...
        for (; l < all_combo_tests; l++) {
            ...
            CHECK(secp256k1_prove_nopenena_outputs(ctx, &nctx, &proof, &w,
                                                 ...
                                                 acc_sk_ptr,
                                                 openout_sk_ptr,
                                                 j_index, p_size, acc_size, uproof_ptr, nonce, NOPENI_ALL, zero_com_flag) == 1);
            ...
            for (int trial = 0; trial < 3; trial++) {
                ...
                CHECK(secp256k1_verify_nopenena_outputs(ctx, &nctx, &proof, &w,
                                                      asset1_ptr, p_size, acc_size, uproof_ptr, NOPENI_ALL, zero_com_flag) == 1);
...
void bench_nopenena_split_t(int zero_com_flag, int n) {
    int P[] = {1, 1, 1, 1, 1, 1};
    int A[] = {4, 8, 12, 16, 20, 24};
    for (int test = 0; test < 6; test++) {
        int p_size = P[test]; // power of two only
        ...
        for (; l < all_combo_tests; l++) {
            ...
            CHECK(secp256k1_prove_nopenena_balance(ctx, &nctx, &pi_balance,
                                                 ...
                                                 values, values_new,
                                                 acc_size, r, nonce, nonce1,
                                                 &w.C_bin[p_size], alpha_c, (uint64_t)10,
                                                 NULL, NULL, 0) == 1);
            time_t end = clock();
            double prove_all = ((double) (end - start)) / CLOCKS_PER_SEC;
            ...
            for (int trial = 0; trial < 1; trial++) {
                ...
                CHECK(secp256k1_verify_nopenena_balance(ctx, &nctx, &pi_balance,
                                                      0, (uint64_t)9,
                                                      acc, asset0_ptr, asset1_ptr,
                                                      acc_size, &w.C_bin[p_size],  NULL) == 1);
            ...
            printf("%d, %d, %d, %d, %d, %f, %f, %d ,%ld, %ld, %ld\n", acc_size, p_size,
                   secp256k1_nopenena_get_total_combo(p_size, acc_size), nctx.rctx.n, nctx.rctx.m,
                   prove_all, verify_all/1, proof.pi_zero_len, w.pi_len, txsize_without_pi, proof.pi_zero_len + txsize_without_pi);
...
#endif //SECP256K_NOPENI_BENCH_NOPENI_H
